...
def email_me_on_failure(msg, issue, city=city):
    FROM = "{}@gmail.com".format(me)
    TO = ["{}.work@gmail.com".format(me)]
    SUBJECT = "{} for {} data".format(issue, city)
...
lat, lon = cities_lat_and_long[city]
...
# output_file = '{}/{}.txt'.format(data_dir, city)
...
logfile = open('{}/{}_log.txt'.format(logs_dir, city), 'w')
...
while tries <= 20:
    ...
    if err:
        msg = "{}; {}; {}; curl ERROR {}\n".format(hostname, now_time, city, err)
        logfile.write(msg)
        if tries == 15:
    ...
    try:
        ...
        json_data.update({'time_stamp': current_ts, 'city': city})
        ...
        prod.send_messages(topic, json.dumps(json_data))
        # outfile.write(json.dumps(json_data))
        # outfile.write('\n')
        tries = 0
    except Exception as e:
        msg = "{}; {}; {}; Exception {}".format(hostname, now_time, city, e)
        try:
