...
def date_type_post():
    ...
    # for val in response:
        # response_list.append(val)
    # jsonresponse = [{"date": x.date, "type": x.type,
                     # "city": x.city, "count": x.count}
                    # for x in response_list]
    # print jsonresponse
    ...
    date = str(date)
    kwargs = dict(
        title = {"text": 'Reports for {} events'.format(type_id)},
        subtitle = {"text": date},
    ...
    kwargs.update(series_name=series_name, series_data=series_data)
    # print kwargs
